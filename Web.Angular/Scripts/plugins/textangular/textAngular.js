		...
		link: function(scope, element, attrs, controller){
			...
			if(_isInputFriendly){
				...
				if(!_isContentEditable){
					...
					var processpaste = function (event, text) {
						...
						if(text && text.trim().length){
							...
							if(_pasteHandler) text = _pasteHandler(scope, {event: event, $html: text}) || text;
					...
					element.on('paste', scope.events.paste = function (e, eventData) {
						...
						if (clipboardData && clipboardData.getData && clipboardData.types.length > 0) {// Webkit - get data from clipboard, put into editdiv, cleanup, then cancel event
							...
							$timeout(function(){
								...
								processpaste(e, _tempDiv[0].innerHTML);
								element[0].focus();
								_tempDiv.remove();
							}, 0);
						}
	...
	function($compile, $timeout, taOptions, taSelection, taExecCommand,
		...
		return {
			...
			link: function(scope, element, attrs, ngModel){
				...
				if(attrs.taPaste){
					scope._pasteHandler = function(_event, _html){
					    return $parse(attrs.taPaste)(scope.$parent, {event: _event, $html: _html});
					};
					scope.displayElements.text.attr('ta-paste', '_pasteHandler(event, $html)');
				}
